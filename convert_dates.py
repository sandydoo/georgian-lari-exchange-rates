...
for currency in args.currency:
    CSV_URL = f'https://nbg.gov.ge/gw/api/ct/monetarypolicy/currencies/export/csv?currencies={currency}&end={to_iso(end_date)}&start={to_iso(offset_start_date)}'
    headers = { 'Accept-Language': 'en' }

    print(f'Fetching CSV for {currency} from {start_date} to {end_date}...')
    df = pd.read_csv(
            CSV_URL,
    ...
    df.to_csv(f'{currency}-{year}-{month}.csv')
    print('Done!')
